...
void WardenMgr::Update(uint32 diff)
{
    ...
    if (m_PingTimer.Passed())
    {
        ...
        {
            sLog->outError("Connection to Warden Daemon was lost, trying to reconnect in the background");
            m_connector.close();
            m_PingTimer.Reset();
...
void WardenMgr::Update(WorldSession* const session)
{
    ...
    if (session->m_WardenTimer.Passed())        // We don't care the connection to warden state to do cheat-checks or register
    {
