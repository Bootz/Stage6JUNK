...
void WardenSocket::OnRead()
{
    ...
    while (1)
    {
        ...
        {
            char sign[7];
            if(!recv_soft(sign, 7) || strcmp(sign, WARDEN_SIGN))
            {
                sLog->outStaticDebug("Received a connection not form worldserver '%s'.", sign);
                recv_skip(recv_len());
                return;
            }
            _connected = true;
        ...
        {
            sLog->outStaticDebug("[Wardenserver] got unknown packet %u, len of received data %u", (uint32)_cmd, (uint32)recv_len());
            return;
        }
    }
}
...
bool WardenSocket::_HandleLoadModule()
{
    ...
    ByteBuffer pkt;
    if (sWarden->LoadModuleAndExecute(accountId, moduleLen, module, sessionKey, packet, &pkt))
