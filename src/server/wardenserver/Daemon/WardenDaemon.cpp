...
void CWardenLoader::UnloadWarden()
{
    if (pdwModule)
    {
        ...
        {
            dwLibraryCount--;
            sLog->outStaticDebug("Unloading lib %s...", GetAddress<const char*>(pLibraryList[dwLibraryCount].dwFileName));
...
bool CWardenLoader::LoadWarden(PDWORD pdwNewModule, DWORD dwSize)
{
    ...
    if (dwModuleSize < 0x80000000)
    {
        ...
        if (pdwModule)
        {
            ...
            {
                sLog->outStaticDebug("Loading lib %s...", GetAddress<const char*>(pLibraryTable[dwLibraryIndex].dwFileName));
...
{
    sLog->outStaticDebug("Callback cSendPacket called. Packet at 0x%08X size %u.", (PDWORD)pPacket, dwSize);
}

BOOL __stdcall cCheckModule(BYTE* pModName, DWORD _2)
{
    sLog->outStaticDebug("Callback cCheckModule called...");
    return 0;
}

WardenFuncList** __stdcall cLoadModule(BYTE* pRC4Key, BYTE* pModule, DWORD dwModSize)
{
    sLog->outStaticDebug("Callback cLoadModule called...");
    return 0;
}
...
LPVOID __stdcall cAllocateMemory(DWORD dwSize)
{
    ...
    sLog->outStaticDebug("Callback cAllocateMemory called. Allocated %u bytes at 0x%08X", dwSize, res);
    return res;
}

VOID __stdcall cReleaseMemory(LPVOID lpMemory)
{
    sLog->outStaticDebug("Callback cReleaseMemory called. Memory at 0x%08X", lpMemory);
    free(lpMemory);
}

VOID __stdcall cSetRC4Data(LPVOID lpKeys, DWORD dwSize)
{
    sLog->outStaticDebug("Callback cSetRC4Data called. Keys at 0x%08X, key size 0x%04X (%u).", lpKeys, dwSize, dwSize);
}

DWORD __stdcall cGetRC4Data(LPVOID lpBuffer, LPDWORD dwSize)
{
    sLog->outStaticDebug("Callback cGetRC4Data called. Buffer at 0x%08X, key size 0x%04X (%u).", (PDWORD)lpBuffer, *(DWORD*)dwSize, *(DWORD*)dwSize);
...
WardenFuncList **CWardenLoader::InitializeWarden(InstanceS *instance)
{
    ...
    sLog->outStaticDebug("Warden module mapped to: 0x%08X", pdwModule);
    ...
    sLog->outStaticDebug("Initialize Function mapped to: 0x%08X (0x%04X)", (PDWORD)A ,PDWORD((PDWORD)A - pdwModule));
    ...
    char * szCallbacksDesc[] = {"SendPacket", "CheckModule", "LoadModule", "AllocateMemory", "ReleaseMemory", "SetRC4Data", "GetRC4Data"};

    sLog->outStaticDebug("Warden callbacks:");
    for (int i = 0; i < 7; ++i)
        sLog->outStaticDebug("Callback: %s at: 0x%08X" ,szCallbacksDesc[i], ((PDWORD*)&instance->dwTable)[i]);

    // warden functions pointers
    sLog->outStaticDebug("Calling Warden::Initialize");
    WardenFuncList **list = fpInitializeModule((DWORD*)&instance->tableptr);

    char * szInitReturnDesc[] = {"GenerateRC4Keys", "UnloadModule", "PacketHandler", "Tick"};

    sLog->outStaticDebug("Warden::Initialize returns:");
    for (int i = 0; i < 4; ++i)
        sLog->outStaticDebug("Init: Warden::%s at: 0x%08X", szInitReturnDesc[i] ,((PDWORD*) *list)[i]);
...
{
    char * szHeaderDescription[] = {"Module size", "Destructor", "SizeOfCode", "RelocationCount", "ProcedureTable", "ProcedureCount", "ProcedureAdjust", "LibraryTable", "LibraryCount", "ChunkCount"};

    for (int i = 0; i < 10; ++i)
    {
        sLog->outStaticDebug("%u .+ 0x%02X %s -> %u", i ,i * 4, szHeaderDescription[i], pdwHeader[i]);
    }
}
...
{
    sLog->outStaticDebug("Warden::PacketHandler called...");
    ((fnPacketHandler)instance->pointers.f3)(instance->list, pPacket, dwSize, dwBuffer);
    sLog->outStaticDebug("Warden::PacketHandler handeled %u bytes of the packet.", *dwBuffer);
}

void Tick(DWORD _2, InstanceS *instance)
{
    sLog->outStaticDebug("Warden::Tick called...");
    ((fnTick)instance->pointers.f4)(instance->list, _2);
}

void Warden::Initialize()
{
...
uint8* Warden::GenerateNewKeys(InstanceS *instance, uint8* sessionKey/*[40]*/, uint8* packet/*[17]*/)
{
    sLog->outStaticDebug("Warden::GenerateNewKeys()");
